...
import { Cake, Syringe, FileText, TrendingUp, Activity, Clock, CheckCircle, AlertCircle, Calendar as CalendarIcon } from "lucide-react";
...
export default function StatsGrid({ child, milestones = [] }: StatsGridProps) {
  ...
  const nextDate = next
    ? new Date(next.dueDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })
    : "—";
  const nextLabel = next ? next.vaccineName || next.title : null;

  const growthMilestones = milestones.filter((m) => m.category === "GROWTH");
  const completedGrowth = growthMilestones.filter((m) => m.status === "COMPLETED").length;

  // Fetch actual health records
  useEffect(() => {
    ...
    const fetchHealthRecords = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Use stats.totalRecords if available, otherwise fall back to array length
  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <div className="rounded-2xl border border-teal-100 bg-white p-4 shadow-sm hover:shadow-md transition-shadow duration-300">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Developmental milestones</p>
          </div>
        ...
        {/* Growth Progress */}
        <div className="rounded-xl bg-gradient-to-br from-teal-50 to-emerald-50 p-3 border border-teal-100 mb-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              <TrendingUp className="h-3.5 w-3.5 text-teal-600" />
              <span className="text-xs font-semibold text-teal-900">Growth Milestones</span>
            </div>
            <span className="text-xs font-bold text-teal-700">{completedGrowth}/{growthMilestones.length}</span>
          </div>
          <div className="h-1.5 rounded-full bg-white/70 overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-teal-500 to-primary transition-all duration-500"
              style={{ width: `${growthMilestones.length > 0 ? (completedGrowth / growthMilestones.length) * 100 : 0}%` }}
            />
          </div>
          <p className="text-[10px] text-teal-700 mt-1">
            {growthMilestones.length === 0 ? "No milestones tracked yet" : `${completedGrowth} of ${growthMilestones.length} achieved`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-emerald-50 p-2 text-center border border-emerald-100">
            <CalendarIcon className="h-3.5 w-3.5 text-primary mx-auto mb-0.5" />
            <p className="text-[10px] text-slate-500">Next Checkup</p>
            <p className="text-xs font-semibold text-slate-900">Soon</p>
          </div>
          <div className="rounded-lg bg-teal-50 p-2 text-center border border-teal-100">
            <FileText className="h-3.5 w-3.5 text-teal-600 mx-auto mb-0.5" />
            <p className="text-[10px] text-slate-500">Records</p>
            <p className="text-xs font-semibold text-slate-900">{totalRecords}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-cyan-100 bg-white p-4 shadow-sm hover:shadow-md transition-shadow duration-300">
        ...
        {totalRecords > 0 ? (
          <div className="space-y-2 mb-3">
            ...
            {recentRecords.map((record, idx) => (
              <div
                key={record._id || idx}
                className="flex items-center gap-2 rounded-lg bg-cyan-50/50 p-2 hover:bg-cyan-50 transition-colors cursor-pointer border border-cyan-100"
              >
        ...
        {/* Action */}
        <a
          href={`/dashboard/records${child?.registrationId ? `?registrationId=${child.registrationId}` : ""}`}
          className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 border border-emerald-100 transition-colors hover:bg-emerald-100"
        >
          <FileText className="h-3.5 w-3.5" />
          View All Records
        </a>
